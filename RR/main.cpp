...
//        }
        ...
        if(runningProcess.quantumTime == 0){
            if(runningProcess.PID != 0){
                ...
                outFile << "PID " << runningProcess.PID << " " << runningProcess.burstTime << " placed on Ready Queue" << endl;
            }
...
void Create(int n, int burstTime, ofstream &outFile, int q){
    ...
    outFile << "PID " << process.PID << " " << process.burstTime << " placed on Ready Queue" << endl;
...
void waitProcess(int n, PCB &runningProcess, ofstream &outFile){
    ...
    outFile << "PID " << runningProcess.PID << " " << runningProcess.burstTime << " placed on Wait Queue" << endl;
    runningProcess = PID0;
...
void Event(int n, ofstream &outFile){
    ...
    for(int i=0; waiting.size(); i++){
        if(waiting[i].waitingEvent == n){
            ...
            waiting.erase(waiting.begin()+i);
            outFile << "PID " << waiting[i].PID << " " << waiting[i].burstTime << " placed on Ready Queue" << endl;
        }
    }
}
...
void Display(ofstream &outFile){
    ...
    outFile << "Wait Queue: ";
    for(int i=0; i < waiting.size(); i++){
        outFile << "PID " << waiting[i].PID << " " << waiting[i].burstTime << " " << waiting[i].waitingEvent;
    }
    outFile << endl;
}
